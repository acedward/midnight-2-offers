...
6. **`offer-poster`** — started **last**, on its own, only after every other service in the
   bring-up (node, indexer, kernel, batcher, faucet, solver, aa, shielded-night, price feed) has
   passed its `up.sh` wait. The initial `docker compose up` names every rendered service except
   it; `up.sh` then runs `up -d --no-deps offer-poster` and waits for `/health`. Its first posted
   offer is therefore the end-to-end signal that the whole stack works, not a race against it.
