...
CI_IMAGE_TAGS=(
  # ONE aa-contracts tag since 00034 (see pick-ports.sh); the two older names are kept in
  # the list so a stale .env that still sets them is cleaned up too.
...
' "$REPO_ROOT/wallets/wallets.json" "${CI_FUND_PROBE_WALLET:-demo-carol}" 2>/dev/null || true)"
  ...
  # ── step 4f: the EVM-signed PASSPORT ACCOUNT path ─────────────────────────
  # register (deploy an account contract) -> fund -> offer -> a taker settles -> the account
  # spends what it received. Five proofs, the heaviest of them the k=18 offer circuit, so it
  # is last: everything cheaper has already reported by then. It no longer builds an image
  # variant of its own — one aa-contracts image serves deploy, console and e2e.
  if (( ! FAILED && DO_AA_E2E )) && [[ "$PROFILE_MODE" == "all" ]]; then
    step 4f "aa-e2e.sh (register -> fund -> offer -> settle -> spend)" \
